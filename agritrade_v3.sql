...
`answer_id` INT NOT NULL COMMENT '关联的回答ID',
`user_id` INT NOT NULL COMMENT '回复用户ID',
`content` TEXT NOT NULL COMMENT '回复内容',
`replied_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '回复时间',
`upvotes` INT DEFAULT 0 COMMENT '点赞数',
`deleted_by` INT NULL COMMENT '删除管理员ID',
`deleted_at` TIMESTAMP NULL COMMENT '删除时间',
`is_deleted` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否删除',
`delete_reason` VARCHAR(255) NULL COMMENT '删除原因',
PRIMARY KEY (`reply_id`),
INDEX `answer_user_idx` (`answer_id`, `user_id`),
CONSTRAINT `fk_answer_reply`
FOREIGN KEY (`answer_id`) REFERENCES `answers` (`answer_id`)
ON DELETE RESTRICT ON UPDATE RESTRICT,
CONSTRAINT `fk_reply_user`
FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`)
ON DELETE RESTRICT ON UPDATE RESTRICT;
CONSTRAINT `fk_reply_deleted_by`
...
`certificate_name` varchar(255) NULL DEFAULT NULL COMMENT '证书名称',
`certificate_image` varchar(255) NULL DEFAULT NULL COMMENT '证书图片',
 audited_by INTEGER NULL,
     audited_at TIMESTAMP NULL,
     is_audited VARCHAR(20) NOT NULL DEFAULT 'pending' 
        CHECK (is_audited IN ('pending', 'approved', 'rejected')),
     audited_reason VARCHAR(255) NULL;
...
CONSTRAINT certificates_audited_by_fk FOREIGN KEY (audited_by) REFERENCES users (user_id)
 ON DELETE SET NULL ON UPDATE RESTRICT;
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci ROW_FORMAT = Dynamic;
