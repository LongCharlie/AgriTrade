        ...
        <el-main>
          <div class="expert-answer-detail">
                  ...
                  <el-form-item label="回答图片">
                    <el-upload
                      ...
                      <div class="upload-content">
                        ...
                        <div class="upload-hint">最多可上传5张图片，每张不超过5MB</div>
                      </div>
                    </el-upload>
                  </el-form-item>
            ...
            <div class="answers-container">
              <el-card
                  v-for="answer in paginatedAnswers"
            ...
            <!-- 回答分页组件 -->
            <div class="answers-pagination" v-if="answers.length > answersPagination.pageSize">
              <el-pagination
                  @size-change="handleAnswersSizeChange"
                  @current-change="handleAnswersCurrentChange"
                  :current-page="answersPagination.currentPage"
                  :page-sizes="[5, 10, 20]"
                  :page-size="answersPagination.pageSize"
...
export default {
  ...
  computed: {
    // 计算分页后的回答数据
    paginatedAnswers() {
      const start = (this.answersPagination.currentPage - 1) * this.answersPagination.pageSize;
      const end = start + this.answersPagination.pageSize;
  ...
  methods: {
    ...
    async submitAnswer() {
      ...
      try {
        ...
        this.fetchQuestion();
        // 重置分页到第一页
        this.answersPagination.currentPage = 1;
      } catch (error) {
    ...
    // 回答分页相关方法
    handleAnswersSizeChange(val) {
      this.answersPagination.pageSize = val;
      this.answersPagination.currentPage = 1; // 重置到第一页
    },
    handleAnswersCurrentChange(val) {
      this.answersPagination.currentPage = val;
    }
  }
};
</script>
