...
<!--&lt;!&ndash;            <p><strong>有效期：</strong>{{ cert.valid_period }} 年</p>&ndash;&gt;-->
<!--&lt;!&ndash;            <p><strong>描述：</strong>{{ cert.description || '暂无描述' }}</p>&ndash;&gt;-->

<!--            &lt;!&ndash; 证书图片展示 &ndash;&gt;-->
<!--&lt;!&ndash;            <div v-if="cert.image_url" class="certificate-image-container">&ndash;&gt;-->
<!--&lt;!&ndash;              <p><strong>证书图片：</strong></p>&ndash;&gt;-->
<!--&lt;!&ndash;              <el-image&ndash;&gt;-->
<!--&lt;!&ndash;                  :src="`http://localhost:3000${cert.image_url}`"&ndash;&gt;-->
<!--&lt;!&ndash;                  class="certificate-image"&ndash;&gt;-->
<!--&lt;!&ndash;                  fit="cover"&ndash;&gt;-->
<!--&lt;!&ndash;                  :preview-src-list="[`http://localhost:3000${cert.image_url}`]"&ndash;&gt;-->
    ...
    <el-row :gutter="40" class="crop-cards">
      ...
      <el-col :span="8" v-for="(cert, index) in filteredCertificates" :key="index">
        <el-card class="crop-card" shadow="hover" @click="viewCertificateDetail(cert.certificate_id)">
          ...
          <div class="card-footer">
            <span v-if="cert.is_audited === 'rejected'" class="reason-btn" @click.stop="showRejectionReason(cert)">未通过理由</span>
            <span class="delete-btn" @click.stop="confirmDeleteCertificate(cert.certificate_id, cert.certificate_name)">删除</span>
            <span class="edit-btn" @click.stop="editCertificate(cert)">编辑</span>
          </div>
        </el-card>
      </el-col>
        ...
        <el-form-item label="证书图片">
          <el-upload
            ...
            <div class="upload-content">
              <el-icon><Plus /></el-icon>
              <div class="upload-text">{{ hasImageInEdit ? '更换图片' : '点击上传图片' }}</div>
...
export default {
  ...
  computed: {
    ...
    // 判断编辑对话框中是否有图片
    hasImageInEdit() {
      return this.fileList.length > 0 || this.editingCert.image_url;
    }
  },
  mounted() {
    this.fetchCertificates();
  ...
  methods: {
    ...
    // 确认删除证书
    confirmDeleteCertificate(id, name) {
      ElMessageBox.confirm(
          `确定要删除证书 "${name}" 吗？此操作无法撤销。`,
          '确认删除',
          {
